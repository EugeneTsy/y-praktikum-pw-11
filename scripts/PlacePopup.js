...
import { placeError, linkError, loaderInBtn, placeFormSubmitButton, placeNameInput, placeLinkInput, renderCardList } from './script';
import { api } from './API';
...
export class PlacePopup extends Popup {
  ...
  linkValidation(input, errorMessageWrapper) {
    ...
    if (isValid) {
      ...
      this.button.classList.remove('popup__button-plus_active');
    return isValid;
  }
  submit(name, link) {
    ...
    if (isValidPlace && isValidLink) {
      ...
      api.setNewCard(name, link)
        .then((res) => { renderCardList.addCard(res); })
        .catch((res) => console.log(`Не удалось добавить новую карточку. Ошибка: ${res}`));
